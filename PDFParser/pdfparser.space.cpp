...
AffineMatrix::AffineMatrix(std::array<double, 6> coefficients)
    : m_matrix({{coefficients[0], coefficients[1], 0},
...
[[nodiscard]] AffineMatrix
    AffineMatrix::operator*(const AffineMatrix& rhs) const {
	AffineMatrix result;
	result.m_matrix = this->m_matrix * rhs.m_matrix;
