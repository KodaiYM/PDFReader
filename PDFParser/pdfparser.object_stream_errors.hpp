...
namespace pdfparser {
class boolean_object_not_found final: public position_indicatable_error {
public:
	explicit boolean_object_not_found(std::streampos position)
	    : position_indicatable_error(position,
	                                 "Boolean オブジェクトが必要です。") {}
};
class integer_object_not_found final: public position_indicatable_error {
public:
	explicit integer_object_not_found(std::streampos position)
	    : position_indicatable_error(position,
	                                 "Integer オブジェクトが必要です。") {}
};
class real_object_not_found final: public position_indicatable_error {
public:
	explicit real_object_not_found(std::streampos position)
	    : position_indicatable_error(position, "Real オブジェクトが必要です。") {}
};
class string_object_not_found final: public position_indicatable_error {
public:
	explicit string_object_not_found(std::streampos position)
	    : position_indicatable_error(position,
	                                 "String オブジェクトが必要です。") {}
};
class name_object_not_found final: public position_indicatable_error {
public:
	explicit name_object_not_found(std::streampos position)
	    : position_indicatable_error(position, "Name オブジェクトが必要です。") {}
};
class array_object_not_found final: public position_indicatable_error {
public:
	explicit array_object_not_found(std::streampos position)
	    : position_indicatable_error(position, "Array オブジェクトが必要です。") {
	}
};
class dictionary_object_not_found final: public position_indicatable_error {
public:
	explicit dictionary_object_not_found(std::streampos position)
	    : position_indicatable_error(position,
	                                 "Dictionary オブジェクトが必要です。") {}
};
class stream_object_not_found final: public position_indicatable_error {
public:
	explicit stream_object_not_found(std::streampos position)
	    : position_indicatable_error(position,
	                                 "Stream オブジェクトが必要です。") {}
};
class null_object_not_found final: public position_indicatable_error {
public:
	explicit null_object_not_found(std::streampos position)
	    : position_indicatable_error(position, "Null オブジェクトが必要です。") {}
};
class indirect_reference_not_found final: public position_indicatable_error {
public:
	explicit indirect_reference_not_found(std::streampos position)
	    : position_indicatable_error(position, "間接参照が必要です。") {}
};
// TODO: 実装保留中
class specified_object_not_found final: public position_indicatable_error {
public:
	explicit specified_object_not_found(std::streampos position)
	    : position_indicatable_error(position, "message") {}
};

class integer_object_overflows final: public position_indicatable_error {
public:
	explicit integer_object_overflows(std::streampos position)
	    : position_indicatable_error(
	          position, "整数オブジェクトの内部表現がオーバーフローしました。") {}
};

class real_object_overflows final: public position_indicatable_error {
public:
