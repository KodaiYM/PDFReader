...
struct xref_inuse_entry final {
	...
	constexpr bool operator==(const xref_inuse_entry& rhs) const noexcept {
		return std::tie(this->object_number, this->generation_number,
		                this->byte_offset) ==
		       std::tie(rhs.object_number, rhs.generation_number, rhs.byte_offset);
	}
	constexpr bool operator!=(const xref_inuse_entry& rhs) const noexcept {
		return !(*this == rhs);
	}
...
struct xref_free_entry final {
	...
	constexpr bool operator==(const xref_free_entry& rhs) const noexcept {
