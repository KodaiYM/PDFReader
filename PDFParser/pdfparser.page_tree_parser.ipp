...
template <class InputStremT>
    ...
    page_tree_parser<InputStremT>::get_pages() {
...
template <class InputStremT>
	...
	if ("Pages" == type) {
		...
		for (const auto& kid : kids) {
			pages->AddRange(
			    get_pages(m_object_pool.dereference<dictionary_object>(kid),
		...
		page_parser this_page_parser(m_object_pool, complete_page_node);
